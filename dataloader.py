...
def download_oss_file(my_file = "xxx"):

    MY_FILE_PATH = os.path.join(ROOT_DIR, my_file)
    with open(MY_FILE_PATH, "rb") as f:
        content = f.read()
    img = np.frombuffer(content, dtype=np.uint8)  # 注意这里是uint8
        ...
        if len(frame_list) >= select_frame_nums:
            ...
            for x in frame_list[start_frame:start_frame+select_frame_nums]:
                while True:
                    try:
                        temp_image_path = video_id+'/'+str(x)+'.jpg'
                        image = download_oss_file(temp_image_path)
...
def generate_dataset_loader_ddp(cfg, world_size, rank):
    ...
    try:
        ...
        warnings.warn("No train.csv file, using val_id.csv as train.csv")
        df_train = None

    if cfg['task'] == 'normal':
        df_val = pd.read_csv('GenVideo-100K/GenVideo-Val.csv')
