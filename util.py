...
def eval_model(cfg, model, val_loader, loss_ce, val_batch_size, th=0.5):
    ...
    with torch.no_grad():  # No need to track gradients during validation
        for i, (_, input, target, binary_label, video_id) in enumerate(tqdm(val_loader, desc="Validation", total=len(val_loader))):
            ...
            # print(binary_label, var_Binary_Target, logit)  # --- IGNORE ---
...
def eval_model_ddp(cfg, model, val_loader, loss_ce, val_batch_size, rank, world_size, th=0.5):
    ...
    with torch.no_grad():
        for i, (_, input, target, binary_label, video_id) in enumerate(tqdm(val_loader, desc="Validation", total=len(val_loader), disable=rank != 0)):
            if i == 0:
                ss_time = time.time()
    ...
    valLoss = valLoss / lossTrainNorm if lossTrainNorm > 0 else 0

    # Gather results from all processes
    outpred = np.concatenate(outpred_list, 0) if outpred_list else np.array([])
    gt_label = np.concatenate(gt_label_list, 0) if gt_label_list else np.array([])
    video_list = np.concatenate(video_list, 0) if video_list else np.array([])
...
def train_one_epoch_ddp(cfg, model, loss_ce, scheduler, optimizer, epochID, max_epoch, max_acc, train_loader, val_loader, snapshot_path, rank, world_size):
    ...
    disable_tqdm = rank != 0
    pbar = tqdm(total=cfg['bath_per_epoch'], disable=disable_tqdm)
    ...
    trainLoss = trainLoss / lossTrainNorm if lossTrainNorm > 0 else 0
    
    # Synchronize all processes before evaluation
    dist.barrier()
    
    if (epochID+1) % 1 == 0:
        pred_accuracy, video_id, pred_labels, true_labels, outpred = eval_model_ddp(cfg, model, val_loader, loss_ce, cfg['val_batch_size'], rank, world_size)    

        # Only rank 0 saves models and writes results
        if rank == 0:
            torch.save(
                {"epoch": epochID + 1, "model_state_dict": model.module.state_dict()},
                snapshot_path + "/last"+ ".pth",
            )
            ...
            if len(pred_labels) > 0 and len(true_labels) > 0:
                ...
                temp_result_txt = snapshot_path+'/Epoch_'+str(epochID)+'_accuracy.txt'
                with open(temp_result_txt, 'w') as file:
                    true_labels_df = df_result['actual_label']
                    pred_probs = df_result['predicted_prob'] 
                    if len(true_labels_df) > 0 and len(pred_probs) > 0:
                        auc = roc_auc_score(true_labels_df, pred_probs)
                        ap = average_precision_score(true_labels_df, pred_probs)
                        file.write(f"总正确率: {pred_accuracy:.2%}\n")
                        file.write(f"AUC是: {auc:.2%}\n")
                        file.write(f"AP是: {ap:.2%}\n")
                ...
                for index, temp_prefixes in enumerate(prefixes):
                    ...
                    if len(temp_df_val) > 0:
                        ...
                        temp_df_val = df_result[condition]
                        if len(temp_df_val) > 0:
                            true_labels_temp = temp_df_val['actual_label']
                            pred_probs_temp = temp_df_val['predicted_prob']
                            if len(true_labels_temp) > 0 and len(pred_probs_temp) > 0:
                                ap = average_precision_score(true_labels_temp, pred_probs_temp)
                            else:
                                ap = 0
                        else:
                            ap = 0
